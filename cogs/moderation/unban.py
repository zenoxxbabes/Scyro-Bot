...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"<a:alert:1396429026842644584> You have been Banned from **{self.author.guild.name}** by **{self.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        ...
        embed.set_author(name=f"Banned {self.user.name}", icon_url=self.user.avatar.url if self.user.avatar else self.user.default_avatar.url)
...
class Unban(commands.Cog):
    ...
    async def unban(self, ctx, user: discord.User, *, reason=None):
        ...
        if not any(ban_entry.user.id == user.id for ban_entry in bans):
            ...
            embed.add_field(name="__Ban__:", value="> <a:dot:1396429135588626442> Click on the `Ban` button to ban the mentioned user.")
        ...
        embed.set_author(name=f"Unbanned {user.name}", icon_url=self.get_user_avatar(user))
