...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
  ...
  # Logging Integration
  logging_cog = ctx.bot.get_cog("Logging")
  if logging_cog:
      desc = f"**{deleted} messages** were purged in {ctx.channel.mention} by {ctx.author.mention}."
      data = {'description': desc}
      logging_cog.queue_event(ctx.guild.id, 'moderation', data)
      asyncio.create_task(logging_cog.log_to_history(ctx.guild.id, "moderation", "purge", ctx.author.id, ctx.channel.id, f"Purged {deleted} messages"))
...
class Message(commands.Cog):
  ...
  async def _reactions(self, ctx, search=100):
        ...
        await ctx.success(f"<:yes:1396838746862784582> | Successfully removed {total_reactions} reactions.")
