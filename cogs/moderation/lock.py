...
class LockUnlockView(ui.View):
    ...
    async def unlock(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.set_author(name=f"Unlocked {self.channel.name}", icon_url="https://cdn.discordapp.com/emojis/1222750301233090600.png")
...
class Lock(commands.Cog):
    ...
    async def lock_command(self, ctx, channel: discord.TextChannel = None):
        ...
        embed = discord.Embed(
            description=f"> <a:dot:1396429135588626442> **Channel**: {channel.mention}\n> <a:dot:1396429135588626442> **Status**: Locked",
            color=self.color
        )
        embed.add_field(name="> <a:dot:1396429135588626442> **Moderator:**", value=ctx.author.mention, inline=False)
        embed.set_author(name=f"Locked {channel.name}", icon_url="https://cdn.discordapp.com/emojis/1222750301233090600.png")
        embed.set_footer(text=f"Requested by {ctx.author}", icon_url=ctx.author.avatar.url)
        view = LockUnlockView(channel=channel, author=ctx.author, ctx=ctx)  
        message = await ctx.send(embed=embed, view=view)
        view.message = message
