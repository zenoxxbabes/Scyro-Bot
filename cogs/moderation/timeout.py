...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.set_author(name=f"Unmuted {self.user.name}", icon_url=self.user.avatar.url if self.user.avatar else self.user.default_avatar.url)
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        if user.is_timed_out():
            ...
            embed.add_field(name="__Unmute__:", value="> <a:dot:1396429135588626442> Click on the `Unmute` button to remove the timeout from the user.")
        ...
        try:
            await user.send(f"<a:alert:1396429026842644584> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"> <a:dot:1396429135588626442> **Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                          ...
                                          f"> <a:dot:1396429135588626442> **Reason:** {reason or 'None'}\n"
                                          f"> <a:dot:1396429135588626442> **Duration:** {duration_text}",
                              color=self.color)
        embed.set_author(name=f"Muted {user.name}", icon_url=self.get_user_avatar(user))
    ...
    async def mute_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            embed = discord.Embed(title="<a:alert:1396429026842644584> Access Denied", description="I don't have permission to mute members.", color=self.color)
            await ctx.send(embed=embed)
        elif isinstance(error, discord.Forbidden):
            embed = discord.Embed(title="<a:alert:1396429026842644584> Missing Permissions", description="I can't mute this user as they might have higher privileges (e.g., Admin).", color=self.color)
            await ctx.send(embed=embed)
            
        else:
            embed = discord.Embed(title="<a:alert:1396429026842644584> Unexpected Error", description=str(error), color=self.color)
            await ctx.send(embed=embed)
