...
class AntiBan(commands.Cog):
    ...
    async def cog_load(self):
        print("✅ [AntiBan] Extension loaded & DB initialized (MongoDB).")
    ...
    async def on_member_ban(self, guild, user):
        ...
        # Check Module Status
        module_doc = await self.modules_col.find_one({"guild_id": guild.id, "module": "ban"})
        if module_doc and not module_doc.get("enabled", True): # Default to True logic if row missing? SQL code said row[0] if row else True
            return
        if not module_doc: # If no doc, default is True as per previous logic "return row[0] if row else True"
             pass 

        # Enhanced rate limiting
        if not self.can_fetch_audit(guild.id, "member_ban", max_requests=3, interval=5, cooldown_duration=180):
            return
        ...
        # Check if executor is a high-ranking role (admin, etc.) - Only "proceed with caution" comment previously
        try:
        ...
        # Additional check for extra owners
    ...
    async def ban_executor(self, guild, executor, user, retries=3):
        ...
        try:
            ...
            while retries > 0:
                try:
                    await guild.ban(executor, reason="[ANTINUKE] Member Ban | Unwhitelisted User | Security Action")
                    break
                except discord.Forbidden:
                    return
            ...
            # Then, try to unban the original user
            retries = 3
            while retries > 0:
                try:
                    await guild.unban(user, reason="[ANTINUKE] Reverting ban by unwhitelisted user | Security Action")
                    break
                except discord.NotFound:
                    # User is not banned, which is fine
                    break
                except discord.Forbidden:
                    return
                except discord.HTTPException as e:
