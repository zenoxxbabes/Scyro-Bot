...
class AntiIntegration(commands.Cog):
    ...
    async def cog_load(self):
        print("✅ [AntiIntegration] Extension loaded & DB initialized (MongoDB).")
    ...
    async def on_guild_integrations_update(self, guild):
        ...
        # Check Module Status
        module_doc = await self.modules_col.find_one({"guild_id": guild.id, "module": "integration"})
        ...
        if whitelist_doc and whitelist_doc.get("mngweb"): # Using mngweb as integration permission
            return
    ...
    async def ban_executor(self, guild, executor):
        ...
        while retries > 0:
            try:
                await guild.ban(executor, reason="[ANTINUKE] Integration Create | Unwhitelisted User | Security Action")
                return
