...
class AntiWebhookCreate(commands.Cog):
    ...
    async def cog_load(self):
        print("✅ [AntiWebhookCreate] Extension loaded & DB initialized (MongoDB).")
    ...
    def can_fetch_audit(self, guild_id, event_name, max_requests=2, interval=5, cooldown_duration=300):
        # Strict limits: 2 requests per 5s default
    ...
    async def on_webhooks_create(self, channel):
        ...
        # Check Module Status
        module_doc = await self.modules_col.find_one({"guild_id": guild.id, "module": "webhook_create"})
    ...
    async def ban_executor_and_delete_webhook(self, guild, executor, webhook):
        ...
        while retries > 0:
            try:
                await guild.ban(executor, reason="[ANTINUKE] Webhook Create | Unwhitelisted User | Security Action")
                if webhook:
