...
class AntiWebhookUpdate(commands.Cog):
    ...
    async def cog_load(self):
        print("✅ [AntiWebhookUpdate] Extension loaded & DB initialized (MongoDB).")
    ...
    def can_fetch_audit(self, guild_id, event_name, max_requests=2, interval=5, cooldown_duration=180):
    ...
    async def on_webhooks_update(self, channel):
        ...
        if not guild.me.guild_permissions.manage_webhooks or not guild.me.guild_permissions.view_audit_log:
            return
        ...
        # Check Module Status
        module_doc = await self.modules_col.find_one({"guild_id": guild.id, "module": "webhook_update"})
        ...
        if not self.can_fetch_audit(guild.id, 'webhook_update', max_requests=3, interval=5, cooldown_duration=180):
            return
    ...
    async def ban_executor_and_delete_webhook(self, guild, executor, webhook):
        ...
        while retries > 0:
            try:
                await guild.ban(executor, reason="[ANTINUKE] Webhook Update | Unwhitelisted User | Security Action")
                break
            except discord.Forbidden:
                return
        ...
        # Then, try to delete the webhook
        retries = 3
        while retries > 0:
            try:
                if webhook:
                    await webhook.delete(reason="[ANTINUKE] Webhook updated by unwhitelisted user | Security Action")
                return
