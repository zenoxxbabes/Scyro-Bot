...
class AntiEmojiUpdate(commands.Cog):
    ...
    async def cog_load(self):
        print("✅ [AntiEmojiUpdate] Extension loaded & DB initialized (MongoDB).")
    ...
    async def on_guild_emojis_update(self, guild, before, after):
        if len(after) == len(before):  # An emoji was updated
            ...
            # Check Module Status
            module_doc = await self.modules_col.find_one({"guild_id": guild.id, "module": "emoji_update"})
    ...
    async def ban_executor(self, guild, executor):
        ...
        while retries > 0:
            try:
                await guild.ban(executor, reason="[ANTINUKE] Emoji Update | Unwhitelisted User | Security Action")
                return
