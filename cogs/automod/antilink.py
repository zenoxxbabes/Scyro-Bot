...
class AntiLink(commands.Cog):
    ...
    async def cog_load(self):
        print("✅ [AntiLink] Extension loaded & DB initialized (MongoDB).")
    ...
    async def is_anti_link_enabled(self, guild_id):
        doc = await self.rules_col.find_one({"guild_id": guild_id, "rule": "anti_link"})
        return doc and doc.get("enabled", False)
    ...
    async def on_message(self, message):
        ...
        if self.link_pattern.search(message.content):
            ...
            try:
                ...
                simple_embed.description = f"<:yes:1396838746862784582> | {user.mention} has been successfully **{action_taken}** for **Posting a link.**"
