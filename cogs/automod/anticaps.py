...
class AntiCaps(commands.Cog):
    ...
    async def cog_load(self):
         print("✅ [AntiCaps] Extension loaded & DB initialized (MongoDB).")
    ...
    async def is_anti_caps_enabled(self, guild_id):
        doc = await self.rules_col.find_one({"guild_id": guild_id, "rule": "anti_caps"})
        return doc and doc.get("enabled", False)
            ...
            if caps_percentage > self.caps_threshold:
                ...
                try:
                    if punishment == "Mute":
                        ...
                        return # No punishment or None

                    await message.delete()

                    simple_embed = discord.Embed(title="Automod Anti-Caps", color=0xff0000)
                    simple_embed.description = f"<:yes:1396838746862784582> | {user.mention} has been successfully **{action_taken}** for **Excessive caps.**"
