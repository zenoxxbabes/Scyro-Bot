...
class AntiInvite(commands.Cog):
    ...
    async def cog_load(self):
        print("✅ [AntiInvite] Extension loaded & DB initialized (MongoDB).")
    ...
    async def is_anti_invites_enabled(self, guild_id):
        doc = await self.rules_col.find_one({"guild_id": guild_id, "rule": "anti_invites"})
        return doc and doc.get("enabled", False)
        ...
        if invite_match:
            ...
            try:
                ...
                try:
                    ...
                    simple_embed.description = f"<:yes:1396838746862784582> | {user.mention} has been successfully **{action_taken}** for **posting an invite link.**"
