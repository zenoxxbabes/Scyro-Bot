...
class AntiMassMention(commands.Cog):
    ...
    async def cog_load(self):
        print("✅ [AntiMassMention] Extension loaded & DB initialized (MongoDB).")
    ...
    async def is_anti_mass_mention_enabled(self, guild_id):
        doc = await self.rules_col.find_one({"guild_id": guild_id, "rule": "anti_mass_mention"})
        return doc and doc.get("enabled", False)
    ...
    async def on_message(self, message):
        ...
        if mention_count >= self.mass_mention_threshold:
            ...
            try:
                ...
                simple_embed.description = f"<:yes:1396838746862784582> | {user.mention} has been successfully **{action_taken}** for **mass mentioning.**"
